...
def add_image():
    ...
    try:
        ...
        image_dto = ImageDTO(id = None, data=image_schema.data)
        
        added_image_dto = image_repository.add_image(image_dto, product_id)
    except NotFound as e:
        return jsonify({'error': f'{e}'}), 404
    except Exception as e:
        return jsonify({'error': f'{e}'}), 400
    else:
        return jsonify({'new image ID': added_image_dto.map_image_dto_to_json(), 'product ID with new image': product_id}), 201
...
def get_image_by_id():
    try:
        ...
        image_dto = image_repository.get_image_by_id(image_id)
    except NotFound as e:
        return jsonify({'error': f'{e}'}), 404
    except Exception as e:
        return jsonify({'error': f'{e}'}), 400
...
def get_image_by_product_id():
    try:
        ...
        image_dto = image_repository.get_image_by_product_id(product_id)
    except NotFound as e:
        return jsonify({'error': f'{e}'}), 404
    except Exception as e:
        return jsonify({'error': f'{e}'}), 400
